...
def get_border_masks(object_masks, kernel_size=3):
    ...
    for i, mask in enumerate(object_masks):
        ...
        # Create a small border around the object by performing an erosion and
        # a dilation. The erosion ensures we are able to capture the transition
        # from object -> object border -> background
...
def get_stats_all(image, class_mask, area_thresh=300):
    """Returns a centroid, bounding box and contours for a mask containing a
    unique object.

    Parameters
    ----------
    class_mask : array of shape (rows, cols) containing an unique object
    ...
    mask = np.uint8((class_mask / np.max(class_mask)) * 255)
    if SHOWME_OBJ_FLAG == True:
    ...
    _, _, bbox, centroid_list = cv2.connectedComponentsWithStats(mask)
    good_idx = [i for i, row in enumerate(bbox) if 500000 > row[-1] > area_thresh] # max limit was 10000. Do we need a max limit?! TO DO

    if len(good_idx) == 0:
        return None, None, None, None
    good_idx = np.hstack(good_idx)
    centroid_avg = centroid_list
    ...
    if len(good_idx) >= 1:
        ...
        centroid_avg = np.mean(centroid_list[good_idx], axis=0)
        centroid_avg = centroid_avg.astype(int)

        bbox_ = np.zeros((4,), dtype=int)
        bbox_[0] = np.min(bbox[good_idx, 0])
        bbox_[1] = np.min(bbox[good_idx, 1])
        bbox_[2] = np.max(bbox[good_idx, 2])
        bbox_[3] = np.max(bbox[good_idx, 3])
        ...
        if SHOWME_OBJ_FLAG == True: #for debugging purposes
            print "bbox:", bbox_
            img = image.copy()
            cv2.rectangle(img,(bbox_[0],bbox_[1]),(bbox_[0]+bbox_[2],bbox_[1]+bbox_[3]),(255,255,255),2)
            #img = img+mask
            #print "img type", img.shape
            #image[centroid_list[1]][centroid_list[0]]=600
            showme(img, 'Object bbox')

        return list(centroid_avg.flatten()), centroid_list.tolist(), list(bbox_.flatten()), contours
    else:
        return [], [], [], []
    ...
    centroid_list = list((centroid_list[good_idx, :].flatten()).astype(int))

    return centroid_list, centroid_list, bbox, contours


def get_stats( class_mask, area_thresh=300):
    """Returns a centroid, bounding box and contours for a mask containing a
    unique object.
...
def get_object_stats(image, object_masks, object_borders, area_thresh=300):
    ...
    if object_masks.ndim == 2:
        object_masks = object_masks[np.newaxis]
    if object_borders.ndim == 2:
        object_borders = object_borders[np.newaxis]

    # Store information by appending to lists held within a dict
    object_stats = {key : [] for key in ['bbox', 'centroid', 'centroid_list', 'contours']}

    for mask in object_masks:
    #for mask in object_borders:
        centroid, centroid_list, bbox, contours = get_stats_all(image, mask, area_thresh=area_thresh)
        ...
        object_stats['centroid_list'].append(centroid_list)
...
def main():
    ...
    classes_of_interest =['stem', 'tomato','leaf']
    # classes_of_interest must be sorted in reverse order of their index in GLOBAL_CLASSES
    classes_of_interest = [class_name for class_name,_ in sorted(GLOBAL_CLASSES.items(), key=operator.itemgetter(1), reverse=True) if class_name in classes_of_interest ]
    print "classes of interest: ", classes_of_interest
    ...
    for i, f in enumerate(all_files):
        ...
        for class_name in classes_of_interest:
            ...
            #print "dense_mask: ", dense_mask.dtype, dense_mask.shape
            ...
            if class_name in annotated.get_classes():
                ...
                if SHOWME_FLAG == True:
                    showme(class_mask.astype(np.uint8),'class mask for '+class_name)
                ...
                # The class mask is then the portions of class contained in the border
                ...
                if not(class_name in all_stats.keys()):
                    all_stats.update({class_name:{}})
                rgb_image = mpimg.imread(f + '.jpg')
                all_stats[class_name][f] = get_object_stats(rgb_image, objects, borders)
    ...
    # [stem border, stem, tomato border, tomato, leaf border, leaf, bg tomato, bg stem, bg leaf, bg]
    all_images = np.vstack(all_images)
    all_masks = np.vstack(all_masks)
    # print "all_masks shape:", all_masks.shape, all_masks.dtype, np.unique(all_masks)

    np.savez(os.path.join(GLOBAL_SAVE_DIR, 'seg-tomato-leaf-stem-images.npz'), all_images)
    np.savez(os.path.join(GLOBAL_SAVE_DIR, 'seg-tomato-leaf-stem-labels.npz'), all_masks)

    json_path = os.path.join(GLOBAL_SAVE_DIR, 'loc-tomato-leaf-stem-labels.json')
    ...
    for i, img in enumerate(all_images):
        ...
        # print "img:", img.shape
        img = (0.5 * img.transpose(1, 2, 0)).astype(np.uint8) #here changes type from uint to uint8
        overlay = (0.5 * composite).astype(np.uint8) #here changes type from uint to uint8
